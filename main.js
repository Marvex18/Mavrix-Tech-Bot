...
// ✅ AutoReact System Import (ONLY ONCE - NO DUPLICATES)
const { handleAutoReact, handleAutoReactCommand } = require('./lib/autoreact');
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        if (!userMessage.startsWith('.')) {
            ...
            // ✅ AutoReact to non-command messages
            try {
                await handleAutoReact(sock, message);
            } catch (err) {
                console.error('AutoReact runtime error:', err);
            }
            return;
        }
        ...
        const ownerCommands = ['.mode', '.autostatus', '.antidelete', '.cleartmp', '.setpp', '.clearsession', '.autoreact', '.autotyping', '.autoread', '.pmblocker'];
        ...
        switch (true) {
            ...
            case userMessage.startsWith('.autoreact') ||
                 userMessage.startsWith('.areact') ||
                 userMessage.startsWith('.autoreaction'):
                const args = userMessage.replace(/^(\.autoreact|\.areact|\.autoreaction)\s*/i, '');
                await handleAutoReactCommand(sock, chatId, message, args);
                commandExecuted = true;
                break;
        ...
        // ✅ AutoReact to command messages as well
        try {
            await handleAutoReact(sock, message);
        } catch (err) {
            console.error('AutoReact runtime error:', err);
        ...
        // Removed addCommandReaction to avoid conflicts
    } catch (error) {
